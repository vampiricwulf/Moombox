...
| 5 | `grep "triggering recovery" moombox.log` | Absent (healthy). FOUR lines can match, all at Warn: tier 1's three - `youtube auth lost, triggering recovery` and `twitch auth lost, triggering recovery` (both from the periodic validate pass, `refresh.go`) and `twitch credentials were refused where they were used, triggering recovery` (the chat/playback mark, `NoteTwitchAuthLoss`) - and, since the flip, tier 2's `a liveness observation reports this platform is signed out, triggering recovery` (`ObserveLiveness`). | Present without a real outage. The tier-2 sentence names a false liveness verdict (rebuild with the constant back to `false`, then diagnose the page shape); any of the three tier-1 sentences is an A3-class diagnosis. |
