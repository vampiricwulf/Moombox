...
Branch `cookie-arc12a-tui-cookie-login`, cut from `main` @ `15257f3`; the spec and this plan landed as `3aff63a`, the four tasks as `ba52231` (Task 1), `e737b27` + fix round 1 `d16b3ba` (Task 2), `b43f21d` (Task 3), `9ca0199` (Task 4). The Fable arc-close review of `15257f3..9ca0199` is `.superpowers/sdd/2026-09-02-arc12a-tui-cookie-login-and-merge-key/arc12a-arc-close-review.md` (gitignored beside the ledger `progress.md`); it traced R1-R3 as one chain through the TUI and R4 as one chain through the merge by opening the code, checked every doc sentence the tasks changed against HEAD, and ran ten mutations in a `%TEMP%` worktree (eight caught; two survived — both test-precision gaps, carried to `arc12a-close-fix-items.md` with test shapes proven against their mutants). The commit that adds this section also fixes the doc sentences its body names, one of which — where the accepted verdict is rendered — is a residual the arc had not written down.

### What each task delivered

| Task | Commit | Delivered | Review |
|---|---|---|---|
| 1 | `ba52231` | `mergeCookieFiles`' `cookieKey{name, domain, path}` with `path` from `fields[2]` (`internal/cookies/autocookies_merge.go`); `TestMergeCookieFilesKeepsRowsThatDifferOnlyInPath` (two collision subtests + the same-path premise); the twelve "name+domain" mentions across seven files corrected; `data-and-storage.md:872`; the remediation plan's T3 DONE addendum; `jar.go` and the three callers untouched | `task-1-review.md`: PASS / PASS, 3/3 mutants |
| 2 | `e737b27` + `d16b3ba` | `OpenCookieLogin(platform)` — visible-guard, cookie-only, `setupModeSimple`/`setupSimpleCookies`, the row preselect, the nine cookie fields reset, never `Open()`, never the countdown; `closeCookieLogin()` — cancel-if-active then `Close()`, the one funnel; `cookieOnly` on exactly the picker's Esc and its third row; `Open()` resets the flag; the view's `Cookie Login` header, `Close` row and `Esc: Close` hint; six tests | `task-2-review.md`: PASS / PASS WITH REQUIRED EDITS — the drafted `TestOpenClearsTheCookieOnlyFlag` closed the overlay first and so pinned nothing; fix round 1 (test-only) dropped the close and added `TestOpenCookieLoginNeverCallsOpen`; 8 mutants, all caught after |
| 3 | `b43f21d` | `R L` — one `buildMenuItems()` entry gated on `OnStartAutoCookie`, one `dispatchAction` case (`SetSize`, then `OpenCookieLogin(a.statusBar.ReloginPlatform())`, the nil-guard defensive); `StatusBarModel.ReloginPlatform()` (active-gated, YouTube first); four tests incl. the wired/not-wired junction guard; six `user-interfaces.md` sentences, `CLAUDE.md`'s `R` chords line, the V9 DONE addendum; `cmd/` untouched — `SetSetupCallbacks` was already bound unconditionally | `task-3-review.md`: PASS / PASS, 6/6 |
| 4 | `9ca0199` | the `(R L)` hint appended in `renderCookieStatus` at `tierFull` only, once per bar, on `ReloginPlatform()`; `TestReloginBadgeNamesTheChordThatAnswersIt` with the flagged-bar ladder sweep; `user-interfaces.md` cookie-indicators paragraph and the `Re-login required` parity row | `task-4-review.md`: PASS / PASS, 5/5 |

### Deviations from the plan as written

1. **Task 2.** `TestOpenClearsTheCookieOnlyFlag` as drafted called `closeCookieLogin()` before `Open()`, so the flag was already false and `Open()`'s reset was unpinned (the implementer self-reported it; the review confirmed the mutant survives). Fix round 1 dropped the close and added `TestOpenCookieLoginNeverCallsOpen` — a counting `OnCheckFFmpeg` stub plus a seeded `values` entry, the two side effects `OpenCookieLogin`'s own field writes cannot mask — closing the reviewer's second survivor as well.
2. **Task 3.** The remediation plan's V9 bullet had moved from `:2010` to `:2008` (Task 1 appended to T3's bullet first); located by content, edited in place.
3. **Task 4.** Anchors drifted by Task 3's insertions (`status_bar.go:611` → `:635`, `user-interfaces.md:380`/`:841` → `:381`/`:844`); located by content per F7. The "append per platform" mutant was not run separately: the hint is one `if` after both platform switches, structurally incapable of two copies, and the `both` count assertion fires live under mutants 2 and 3.
4. **Arc close.** The plan review's claim that `cookieTickActive = false` in `OpenCookieLogin` is load-bearing is TRUE and UNPINNED (mutant M7 survives every test); the `ytActive` half of `ReloginPlatform`'s gate is unpinned too (M8a survives — the preselect test cannot see it, since an inactive-but-flagged YouTube yields row 0 either way, so the guard belongs beside the badge). Both carried, test-only.
5. **Arc close.** The `R L` paragraph's "the same four-arm verdict rendering" was true of the code and misleading about the operator: the accepted arm is a `setFeedback` line (3 s) behind an overlay that replaces the whole view. Pre-existing since `93aac7b` (Arc 10), shared by first run; the sentence now says where each arm lands.

### Residuals, each with a home

| Residual | Behaviour | Home |
|---|---|---|
| the `dispatchAction` nil-guard is unreachable from the keyboard | with `OnStartAutoCookie` nil the chord is not registered, `processSecondKey` reports `Invalid Chord: R L`, menu and help omit it; the guard answers only a direct caller | plan review F2; the case comment in `app_actions.go`; `TestCookieLoginChordRefusesWithoutTheService`; `user-interfaces.md` `R L` paragraph |
| the hint is `tierFull` only | the first squeeze drops `(R L)` and keeps the alert; the remedy stays in the menu and help; `metricTiers` must narrow monotonically | `renderCookieStatus` comment; `user-interfaces.md` cookie-indicators paragraph; the ladder sweep in `TestReloginBadgeNamesTheChordThatAnswersIt` |
| the jar stays name-keyed | two rows that differ only in path both survive in `cookies.txt` and still load as ONE jar entry (last-wins for equal domains), the same entry the old key kept | `mergeCookieFiles` comment; `data-and-storage.md:872`; the T3 DONE addendum; spec R4 |
| the accepted verdict is rendered where the overlay hides it | error and rejected arms render inline (`errorMsg`); the accepted arm goes to the App feedback line for 3 s while `View` returns the wizard alone — the operator sees the ✓ on the row, never "accepted but not verified"; pre-existing (`93aac7b`), shared by first run | `user-interfaces.md` `R L` paragraph (this commit); a production follow-up — an overlay-visible notice line in `viewSimpleCookies` — belongs to Arc 12b/12c planning, not this arc's test-only fix dispatch |
| `OpenCookieLogin`'s `cookieTickActive` reset is unpinned | a chain armed by Enter, cancelled inline, and closed before its next tick strands the flag (`UpdateComponents` returns at `!visible`); without the reset the next login's `armCookieTick` is refused and the countdown never runs | `arc12a-close-fix-items.md` item 1 (test shape proven against M7) |
| `ReloginPlatform`'s `ytActive` gate is unpinned | dropping it lets an inactive-but-flagged YouTube put `(R L)` on a bar that shows no YouTube alert | `arc12a-close-fix-items.md` item 2 (proven against M8a) |
| the badge's chord text is hand-typed | `status_bar.go` renders the literal `"R L"`; not a registration, but nothing ties it to the chord `buildMenuItems()` registers, so a re-lettering would leave the badge naming a dead chord | `arc12a-close-fix-items.md` item 3 (proven against a `"R X"` badge) |
| the timed-out branch still says `S  Skip` in cookie-only mode | `S` returns to the picker, which is one Esc from closing; the label is the first-run wording | plan review §3 |
| the `R L` case does not clear `a.feedbackMsg` before opening | as `A O`/`A K`; the overlay covers the line | plan review "not changed, deliberately" |
| `TestHelpCoversEveryChord` never sees `R L` | it runs on a bare `NewApp()`; `R L`'s help presence is the wired subtest of `TestCookieLoginChordExistsWheneverTheWizardCanStartALogin` | Global Constraints (F10); the test's comment |
| `Open()`'s own `cookieTickActive` reset is unpinned | pre-existing; first run opens the wizard once per process | this section; no mechanism |
| `OpenCookieLogin`'s visible-guard is silent | a chord over a visible wizard does nothing and says nothing — but while the wizard is visible every key routes to it first (`app_keys.go`), so the guard is unreachable from the keyboard | `OpenCookieLogin`'s doc comment; `TestCookieLoginRefusesToReopenOverAVisibleWizard` |

### The field gate

None by ruling: the chain is in-memory model code pinned by Windows-runnable tests, and its browser leg (`StartSetup` → the countdown → `FinishSetupDetailed` → `CancelSetup`) is the same service the first-run wizard and the dashboard already drive, unchanged here. The settling observation, for the first real `Re-login` badge: `R L` opens on the flagged platform, Enter opens the browser, Enter again extracts and the ✓ appears, Esc closes — and the next `R F` or `R L` is not met with `ErrSetupInProgress`.
