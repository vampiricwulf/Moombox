...
## Final state (arc close, 2026-09-02)

Branch `cookie-a1-linux-reap`, cut from `main` @ `16afaa6`; the six tasks landed as `8db798c`, `d6e0404`, `89eb024` (+ fix round 1 `19fc271`), `822cee1`, `16a9af8`, `2e75655`. The Fable arc-close review of `16afaa6..2e75655` is `.superpowers/sdd/2026-09-02-a1-linux-process-group-reap/a1-arc-close-review.md` (gitignored beside the ledger `progress.md`); it traced R1–R7 as one chain by reading, re-derived the 18 `processJob` call sites across 12 functions, vetted `internal/cookies` under `GOOS=linux GOARCH=amd64` and `GOARCH=arm64`, and ran seven mutations in a `%TEMP%` worktree (six caught; the seventh, a Warn→Debug downgrade of the kill-error log, survived exactly as the carried fix item predicts). The commit that adds this section also fixes the comments below that still said Linux cannot count or drain, and names one residual the arc had not written down.

### What each task delivered

| Task | Commit | Delivered | Review |
|---|---|---|---|
| 1 | `8db798c` | `internal/cookies/job_pgroup.go`, no build tag: `parseProcStatPGID` (last-`)` rule; `Z`/`X` are not members), `pgroupJob` with `adopt`/`queryable`/`activeProcesses`/`killGroup`/`forget`, the `listProcessGroups`/`killProcessGroup` seams defaulting to `errNoProcessTable`; `job_pgroup_test.go` with the fake table, the unreadable table and the group-kill recorder. Inert: no caller outside the file | `task-1-review.md`: PASS / PASS, 7/7 mutants |
| 2 | `d6e0404` | `killProcessTreeUnix` behind the existing `killProcessTree` seam — `adopt` → `killGroup` → `killOneProcess` fallback (today's `proc.Kill()`, now a seam); no `syscall.Kill(-` in code | `task-2-review.md`: PASS / PASS, 7/7 |
| 3 | `89eb024` + `19fc271` | the `killTrackedProcesses` seam (no-op default, correct on Windows); the three ask sites — `cleanupLocked`, `adoptSetupJobLocked`, `closeLaunchJob` as `runWithTimeout`'s deferred teardown — kill-then-close, kill error logged at Warn and never a reason to skip the close; `trackedSetupJob`'s failed-assign drop pinned as the site that must NOT ask; `drainJob`'s three `job.close()` doc lines | `task-3-review.md`: PASS / PASS WITH REQUIRED EDITS — the recorder was order-blind and no test drove the kill error; fix round 1 (test-only) landed the order-aware recorder over real `newProcessJob()` fixtures and `TestAskThenCloseStillClosesWhenTheKillErrors`; 5/5 after |
| 4 | `822cee1` | `browserGoneFrom` split out of `realSetupBrowserGone` against the two methods it uses, so `TestBrowserGoneFromAProcessGroup` runs the reap's own pairing over a `pgroupJob` and a fake table on Windows | `task-4-review.md`: PASS / PASS, 7/7 |
| 5 | `16a9af8` | `job_linux.go` rewritten: `Setpgid: true` beside `Pdeathsig` in `configureCmdSysProcAttr` (the four launch sites unchanged), `readProcProcessGroups`, `sigkillProcessGroup` (third `pgid <= 0` guard), `init()` binding the three seams, five thin forwarders; the opt-in `//go:build linux` smoke test (`MOOMBOX_LIVE_PGROUP=1`, not a CI gate); nine code comments; six `operations.md` sentences; `SPEC.md:680`; the two carried items (`adopt`'s last-adopt-wins sentence + `TestPGroupJobSecondAdoptReplacesTheGroup`; the Task 2 test-comment reword) | `task-5-review.md`: PASS / PASS, 3/3; linux amd64+arm64 vet clean |
| 6 | `2e75655` | remediation plan `:1876`, `:1878-1880` (residual CLOSED, built-not-verified), `:1978`, `:1996`, `:2013`; field-test plan Part 4 row 14 (`:172`) and the Part 7 A1 row (`:302`) | `task-6-review.md`: PASS, 7 sentences true at HEAD |

### Deviations from the plan as written

1. **Task 1.** The `LastIndexByte`→`IndexByte` mutant is caught by the `((sd-pam))` subtest, not the `(Web Content)` one the comment names (that row has one `)`); the "no group adopted" kill subtest needs a compound mutant because `activeProcesses()` carries its own `queryable()` guard — defence in depth, kept.
2. **Task 1 review, finding 1.** A second `adopt` silently replaces the group. Ruled "last adopt wins; callers adopt once" (`killProcessTreeUnix` builds a fresh job per call, `processJob.assign` adopts once); the doc sentence and the pinning test landed in Task 5.
3. **Task 2.** With the leads-its-own-group refusal neutralised the observed failure is "nothing killed", not "a stranger's group signalled" — `adopt` records the input pid, so `killGroup`'s empty-group refusal blocks. The test comment was reworded in Task 5; the assertion was load-bearing either way.
4. **Task 3.** The plan's recorder kept only the job pointer, so a close-before-ask reorder at every site and a skip-close-on-error survived; fix round 1 (`19fc271`) closed both. Residual: `capturingLogger.contains()` merges levels, so a Warn→Debug downgrade of the kill-error log at any ask site survives — carried to the arc-close fix dispatch (`a1-close-fix-items.md`), test-only.
5. **Task 5.** 18 `processJob` invocations across 12 functions, not the 14 the briefs anticipated (`autocookies.go` ×7, `autocookies_chromium.go` ×5, `autocookies_firefox.go` ×6). Two more Windows-only test-file comments and `operations.md:128`'s trailing sentence were flipped beyond the brief (ruled in scope: they became false in the same commit). A session limit interrupted the implementer mid-Step-4; the edits survived on disk and the task resumed.
6. **The `internal/logger` flake.** Task 3's full-suite run once showed `TestSubscriberDropDoesNotBlockBroadcast` failing (a 3 s stress budget) while five agents compiled concurrently. Ruled environmental: green twice alone, the package is not in the diff, the post-merge full-suite gate re-runs it.
7. **Task 5 review, finding 1.** The review brief's "`sigkillProcessGroup` maps ESRCH to already-gone" was the brief's error; the code and the binding brief agree and no spec asks for it (residual below).
8. **Arc close.** The `queryable()`-based order checks are the only tests that see a real handle change state; every other assertion runs over the fake table. `job_windows.go` and `job_other.go` are absent from the diff, as decision 1 promised.

### Residuals, each with a home

| Residual | Behaviour | Home |
|---|---|---|
| pgid recycling | a group id can, after every member exits, name a stranger's group; `killGroup` refuses an unseen or empty group, `killProcessTreeUnix` goes through the same refusals — one window, not closed | decision 7; `job_pgroup.go` `killGroup` doc; `operations.md` §Browser Cookie Acquisition |
| `setsid` reads as GONE | a browser that leaves the group empties it: `known=true, gone=true`, the slot is released after the grace with the browser on screen, nothing is killed, the next finish answers `ErrNoSetupInProgress`; which packagings do it (snap/flatpak wrappers suspected) is unmeasured | spec R7; `job_linux.go` `configureCmdSysProcAttr` doc; `autocookies.go` `setupBrowserGone` bullet; `operations.md` |
| zombies are not members | `parseProcStatPGID` skips `Z`/`X`; a PID-1 Moombox without an init never reaps an orphaned grandchild | decision 8; `job_pgroup.go`; `operations.md` |
| unreadable `/proc` | `known=false` for as long as it stays unreadable — the pre-A1 wedge, released only by `AbandonSetup`/`CancelSetup`; never a release, never a kill | spec R7; `job_linux.go` `readProcProcessGroups`; `operations.md` |
| the drain WAITS on Linux | `drainJob` now waits for whatever outlives the direct child, up to the budget; where nothing does, lap zero as before; no Linux timing recorded | decision 6; `drainJob` doc; `operations.md` `:130`/`:134` |
| crash-time cleanup covers only the direct child | a crashed Moombox signals nothing: `Pdeathsig` reaches the direct child, the group is counted and killed only by a running Moombox, so a wrapper's handoff outlives a crash — the one thing `KILL_ON_JOB_CLOSE` covers that a group cannot; pre-existing, unmeasured | `job_linux.go` `configureCmdSysProcAttr` doc and `operations.md` (both written at the arc close) |
| ESRCH after `killGroup`'s non-empty check | only in the race between the count and the signal; surfaces as one Warn ("may still be running") while the close still happens; not mapped to "already gone" because no spec asks for it | ledger 21:33; `task-5-review.md` finding 1 |
| `AbandonSetup`'s release arm reads the table twice | `setupBrowserGone` then `killGroup`, under one lock; only a `/proc` that flips from unreadable to readable between the two reads could turn the release into a kill — a mount property, practically never | this section; no mechanism |
| the smoke test is not a CI gate | `job_linux_smoke_test.go` is env-gated and the release workflow never runs `go test` | the test's doc; Global Constraints |
| level-blind kill-error assertion | `TestAskThenCloseStillClosesWhenTheKillErrors` cannot see a Warn→Debug downgrade | `a1-close-fix-items.md` item 1 |
| the `/proc` walk every 50 ms during a drain | a few ms per lap; accepted without a profile | `a1-plan-review.md` §5 accepted list |
| `job_other.go`'s comment | says "Linux uses PR_SET_PDEATHSIG" — true, incomplete; the file is a non-goal | `a1-plan-review.md` §4 |

### The field gate

No Linux live gate, by owner ruling (`2026-08-25-cookie-subsystem-remediation.md:2015`). Nothing in this arc has run on a Linux desktop or in a container; the Linux arm is proven by reading and by `GOOS=linux GOARCH={amd64,arm64} go vet ./internal/cookies/`, every decision by the Windows suite over the fake table. The field-test plan's Part 4 row 14 and Part 7 A1 row say built-not-verified. The settling observations, for whoever has a Linux box: `MOOMBOX_LIVE_PGROUP=1 go test -count=1 -run TestLivePgroup ./internal/cookies/` (Setpgid, `/proc`, `kill(-pgid)`), and an abandoned setup in the container released ~60 s after its browser is gone. A wedge that outlives the grace, or a setup released with the browser still on screen, is the field report the ruling names.
