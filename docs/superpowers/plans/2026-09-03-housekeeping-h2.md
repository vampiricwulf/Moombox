...
**ARMING row 12 (`progress-arc8.md`):** HALF PINNED at `787f56e` — the stamp and the tier-2 suppression it produces are asserted while disarmed. The arming commit still owes the double-FIRE assertion (with the constant true, one chat-marked loss reaches `OnRecoveryNeeded` once, not twice), and — per residual 2 below — the "stamps without escalating" half of the same path.
