...
**Shipped, by commit** (branch `cookie-housekeeping-h2`, cut from `main` @ `6d40b30` — the Arc 12c merge; fifteen commits `7b93c60..8ce95e2`, then this arc-close commit):

| Task | Commit(s) | What landed |
|---|---|---|
| — | `7b93c60` | This plan and its spec (R1-R8), with the Fable plan review applied in place. |
| 1 (R1) | `1559a36` + fix `f8caa96` | The tier-2 re-alarm backs off per platform: `livenessRefireFactor` = 2 and `livenessRefireCap` = 24 h beside `livenessRefireWindow` (30 min, the base); a fourth map `livenessRefireBackoff` beside the three liveness maps; `livenessRefireWindowFor` / `escalateLivenessRefire` / `resetLivenessRefire` (`internal/cookies/refresh.go`), all three called only from `recordLiveness` under `rs.mu`. The first re-alarm lands one base window after the first alarm, each later one doubles, the cap latches, and only a conclusive signed-in verdict resets — the escalation, never `lastRecoveryDecided`. Five tests in `refresh_liveness_test.go` (four symbolic, `TestLivenessRefireScheduleIsTheRuledNumbers` literal); the fix round pinned the two boundaries the review's mutants M-A/M-B had found open (the reset leaves the stamp; an inconclusive probe leaves the back-off) and rewrote `data-and-storage.md` §Refresh Service to today's truth. `const livenessRecoveryArmed = false` is byte-unchanged (its line moved 771 → 810 under added comment lines). |
| 2 (R4 + R6) | `787f56e` | `authStatusChanged`'s body untouched; `TestAuthStatusChangedGateCoversEverySurfaceInput` gains the two rows that move an `Authenticated` boolean ALONE, which is what a gate missing either comparison was passing without. `TestTwitchMarkStampsTheSharedRecoveryDedupe` asserts the stamp `NoteTwitchAuthLoss` writes through `noteRecoveryDecided("twitch", …)` and the tier-2 suppression it produces — the half of ARMING row 12 observable while disarmed. The contract paragraph in `data-and-storage.md` is restated, not widened. |
| — | `c88db6c` | R9 appended to the spec and this plan as Task 7 (the 12c arc-close F1/F2, ruled into H2 as the one REST/web exception). |
| 3 (R2) | `3d1bff8` + fixes `459368a`, `905c234` | `CookieJar.HorizonLogFields()` (`internal/cookies/jar.go`) is the ONE producer of `youtubeAuthHorizon` / `twitchAuthHorizon` / `twitchLoginExpiry` (ISO-8601 UTC or `none`, through `AuthHorizonString`); `TwitchLoginExpiry()` reads the `login` row's expiry without touching `twitchAuthCookieNames` (Q7). The two consumers: `cookiesLoadedFields` (`cmd/moombox/services.go`, the boot line, now seven fields) and `refreshCookiesDetailed`'s `cookie refresh succeeded` line — the success arm, after the write and the jar reload, and NOT the per-launch `autocookies_firefox.go` line (which fires before the merge). `twitchLoginPrunedFromMerge` (`internal/cookies/autocookies_merge.go`) fires the ONE Warn when the expiry prune drops `login` while `auth-token` survives; it names no value. The three "no UI or log carries a horizon" sentences are gone and the settling observation reads "read the two log lines". The fix rounds pinned the success line's three keys and two values (the round-1 survivor), corrected the "only completion point" over-claim in both docs and in the arm's own comment, and added `TwitchLoginExpiry` to the nil-safe list. |
| 4 (R3) | `8205c54` + fix `2326968` | The browser path rolls back a regression, and only a regression (Q13 revised): `regressedAfterWrite` (`before.ok() && after.state == verifyFailed`) shared by `platformsToRestore` (import, both arms) and the new `platformsToRestoreAfterBrowserRefresh` (browser, that arm alone), selected at one site by `importedFromProfile`; the `pre` snapshot runs on both paths whenever a `cookies.txt` exists. Two operator strings and three log/error strings became path-true. The fix round pinned the absent-from-`pre` cell at the unit and integration level (`TestBrowserRefreshWithNoPreviousCookiesKeepsTheFetchedRows`), pinned the browser-path `LastError` sentence by exact equality, made five comments path-true, and hedged the DPAPI-fallback sentence by ruling (comment only). |
| 5 (R5 + R8) | `2b095b6` + fix `67ff292` | `appFeedback{msg, sev, until}` replaces the three `App` fields; the three setters replace the whole value and `clearFeedback` is the only clear; behaviour byte-identical (14 files, every read site renamed to the same field). The wizard's accepted verdict renders inside the overlay on `successMsg` in `SuccessStyle` (both cookie views) AND still reaches the feedback line, by decision. The fix round added the navigation-clear, advanced-view and outlives-the-feedback-line tests the review's mutants had found open. |
| 6 (R7) | `dda6471` | `cmd/moombox/recheck_callsite_test.go` walks `monitor_callbacks.go`, `services.go`, `tui_wiring.go` and pins the five `recheckAfterCookieWrite` sites by shape: four inside a `*ast.DeferStmt`, the `OnPassCompleted` one wrapped by `postRefreshRecheckHook` and assigned to that selector; a multiset equality over the five gestures, never a substring. |
| 7 (R9) | `9c2c301` + fix `8ce95e2` | The boot line: `NewAutoCookieService` computes the launch guard's verdict silently and `AutoCookieService.LogProfileDirVerdict()` says it once from `initServices` after `AcquisitionMode` is wired — ERROR under `auto` with `main`'s message and `"err"` argument verbatim (pinned byte-for-byte), one INFO under `profile`; `TestProfileDirVerdictIsLoggedAfterTheModeIsWired` pins the call ORDER by AST. The post-flight sentences: `RefreshResult.Mechanism` (`"browser"` / `"profile-import"` / `""`) stamped at the `importedFromProfile` decision and carried to all 18 returns by a named result and one `defer`; the additive wire key `mechanism`; one `cookieRefreshMechanismLabel` per surface (`internal/tui/app_actions.go`, `web/public/modules/utils.js`), result first, mode as the fallback, pinned to exact equality across surfaces through goja. The fix round rewrote the `R F` outcome table, pinned an ABORT exit's stamp through the launch guard's own refusal (the review's M-A), and gave the JS the trade-off comment. |
| close | (this commit) | This section; `user-interfaces.md:703` ("Four of the five" → "Five of the seven"; the call expression as shipped, `a.cookieAcquisitionMode()`) and the `R L` paragraph (the accepted arm now renders inside the overlay — its "only through the dashboard today" was made false by R8); the umbrella plan's `:1988` F2 note gains its BUILT tail; the field-test plan's six stale sentences (Part 1's "re-fires every 30 minutes forever" and item 2's "what the code does today"; Part 2 step 1 and Part 5 row 17's boot line, now seven fields; A5's armed cadence; Part 4 gate 7's "no production log line") and two new Part 5 rows (32: the wizard verdict; 33: the `login`-prune Warn) — numbered past `main`'s H1 row 31 so the merge keeps all three. |

**Rulings recorded on this branch:** Q1 back-off (base 30 min, ×2, cap 24 h, reset on a conclusive positive; reset trigger = the tier-2 conclusive positive only; a tier-1 fire stamps without escalating); Q5/Q7 (timestamps only; `authCookieNamesFor` NOT split; `AuthCookieHorizonFor`'s existing callers NOT swept; the second line is the refresh's success arm, not the per-launch line); Q13 revised (the regression arm only on the browser path; the `verifyUnknown` arm never restores there; the DPAPI-fallback sub-path takes the browser policy too, comment hedged, no behaviour change); R8 sets BOTH surfaces (no pending-feedback field, no drain hook); R9 is the one REST/web exception (one additive key, one `utils.js` export, the toast subjects — no route, no status code, no config key, no schema); the Task 3 round-1 survivor was RULED a required pin (the "no seam" premise was false); the `TestFeedbackColorWarningMessages` rows stay real strings because the label resolves to `Browser cookie refresh` under `auto`; the Task 7 re-review's remaining sentence was ruled into this commit rather than a fix round 2.

**What actually changed in behaviour (arc-close trace):**

- **R1 moves state, not calls.** While `livenessRecoveryArmed` is false, `ObserveLiveness` (`internal/cookies/refresh.go`) logs `wouldFireRecovery` and returns at its gate before `OnRecoveryNeeded`; the only other callers of that hook are the tier-1 status block and the Twitch mark's fire path, neither of which reads tier 2's `due`. So the back-off can change exactly one observable today: the `wouldFireRecovery` value on the `liveness observation` line after a re-alarm, which is what the pilot reads. Arming stays a one-constant change. The `-race` run over the package is clean.
- **R3 is the arc's one write-path behaviour change**: a periodic browser pass on an install that already has credentials now verifies each platform BEFORE the write (up to two extra round trips) and, when a platform that verified before is conclusively rejected after, writes the previous rows back and re-verifies them. An inconclusive post-check on the browser path commits the fresh set (the import path still restores there). Of the 18 (before × after) cells per platform the browser policy restores in exactly one.
- **R2 and R9 change log/UI wording and add log fields; nothing branches on them.** `Mechanism` has three non-test readers (the stamp, the wire map, the label's argument) and no predicate. Every new or changed log line carries timestamps, cookie NAMES, counts, platforms, a mechanism label or an already-logged path — never a value (the arc-close log-hygiene table is in `h2-arc-close-review.md`).
- **R5 is byte-identical** (every test file normalises to its pre-fold version under the field rename); **R8 adds one rendered line**; **R4, R6, R7 are tests only**; `cmd/moombox/main.go` has an empty diff; no goroutine, no interface extraction, LF on all 47 blobs, gofmt clean.

**ARMING row 12 (`progress-arc8.md`):** PINNED. Half of it landed at `787f56e` — the stamp and the tier-2 suppression it produces, both asserted while disarmed; the other half landed with the arming commit of 2026-09-03, which flipped the constant and added `TestTwitchMarkAndTierTwoFireRecoveryOnce` (`internal/cookies/refresh_twitch_mark_test.go`): one chat-marked Twitch loss reaches `OnRecoveryNeeded` once, the tier-2 verdict that follows it inside the window reaches it zero more times, and the re-alarm lands only once the back-off window has passed. The "stamps without escalating" half of the same path (residual 2 below) was closed separately by the fix dispatch that followed this arc.

**Residuals, each homed here unless stated:**

1. R1: `livenessRefireWindowFor`'s base fallback is indistinguishable from `defaultRefreshInterval` by test (both 30 min today; Task 1 F6, mutant M-D survives) — the constant's own doc names the coincidence; the shape tests fail the moment either number moves.
2. R1/R6: "a tier-1 fire stamps the dedupe without escalating" (`noteRecoveryDecided`'s doc, `data-and-storage.md` §Refresh Service twice, spec R1) is UNPINNED — the arc-close mutant that makes `noteRecoveryDecided` also call `escalateLivenessRefire` passes the whole `internal/cookies` package. Cost if it lands: a platform the Twitch mark or the tier-1 check already alarmed is pushed onto a longer tier-2 schedule; a second mark in the same outage doubles it. **Open at arc close — closed by the fix dispatch that follows this commit** (one RLock-guarded read of `livenessRefireBackoff["twitch"]` after `NoteTwitchAuthLoss`, asserting no entry).
3. R6: the mark's stamp time source is unpinned (Task 2 mutant B: a `time.Time{}` stamp survives) — by construction of the test, which reads the stored stamp back; informational.
4. R2: the horizons ride the SUCCESS arm only; the two Warn arms downstream of the same write carry none — stated in `data-and-storage.md` §Cookie Jar, `platform-services.md` §Twitch Authentication and the field-test plan's gate 7.
5. R2: `HorizonLogFields` takes three separate `RLock`s (Task 3 F3) — a `Load` landing between them can produce a line mixing pre- and post-swap horizons; no race, no deadlock, and the Twitch-vs-Twitch comparison the settling observation makes is unaffected.
6. R2: `prepareCookieImport` and `FinishSetup` also call `mergeCookieFiles` and can prune the `login` with no Warn — by ruling Q7 the refresh is the one site; `user-interfaces.md` scopes it ("the refresh logs a single Warn"); field-test row 33 says so.
7. R2: a fetched `login` row with an EMPTY value makes the Warn assert a prune that did not happen (Task 3 F5) — narrow; the operator consequence (anonymous chat) is identical.
8. R3: the Warn's `source` log key is unpinned (Task 4 M9a) — log key only; the operator-facing `LastError` sentence is pinned by exact equality.
9. R3: the Chromium DPAPI fallback (`autocookies.go`, the `dpapiExtractAsNetscape` sub-path) takes the browser policy although its rows come from the user's own profile — by ruling (the policy keys on `importedFromProfile`); `platformsToRestoreAfterBrowserRefresh`'s comment names it.
10. R3: an EMPTY browser profile makes `pre` and `post` two checks of the same credentials seconds apart; a genuine OK→Failed flip inside that window triggers a no-op restore and the "refreshed browser cookies did not verify" status about a pass that refreshed nothing. Harmless and rare.
11. R5: `clearFeedback` clearing only `.msg` is structurally unobservable (both setters replace the whole value) — accepted (Task 5).
12. R5: `View()`'s use of the STATED severity is unpinned at the render level — the arc-close mutant `feedbackColor(a.feedback.msg, severityUnstated)` in `app_layout.go` passes the whole `internal/tui` package, because every colour test calls `feedbackColor` on the pair directly rather than reading the rendered overlay. Pre-existing (the pre-fold site had the same shape and the same tests); closable by one test that renders `View()` and checks the overlay line's ANSI colour.
13. R7: the `OnPassCompleted` pin requires an inline `FuncLit` — a named method passed through `postRefreshRecheckHook` fails it although still recover-guarded (Task 6); `recheckSitesIn`'s parse-error `t.Fatalf` is unreachable in practice (a syntax error fails the package build first).
14. R9: the profile INFO line's heading tone is pinned by two `Contains` and the absence of "refusing to launch" (Task 7 M-B survives by design); `TestAutoCookieRefreshToastsNameTheMechanismThatRan` reads `app.js` by text (count ≥ 5, no literal subject), since `autoCookieRefresh` is a class method on a live DOM — the subject producer itself is goja-pinned.
15. `TestRefreshCookiesDetailedReportsPerPlatform` tripped once under load in the Task 1 re-review and in no other run of this arc (fourteen full `internal/cookies` runs at arc close, `-race` included) — pre-existing; the Global Constraints' re-run-once rule applies.
16. The field-test plan's Part 1 still cites `refresh.go:606`/`:632` for the constant (it is `:810` at `8ce95e2`); its own "always re-derive with grep" rule covers it.

**Merge shape (the controller's, at the merge gate).** `main` is at `f05cd24` (H1 merged). A trial `git merge --no-commit f05cd24` onto `8ce95e2` in a throwaway worktree auto-merged with NO conflict hunks: the four shared files (`docs/spec/platform-services.md`, `security.md`, `user-interfaces.md`, the field-test plan) are edited at non-adjacent lines by the two branches, and H1's `internal/docs` checker passed on the merged tree (six tests, all run). THIS commit appends field-test Part 5 rows 32-33 at the table's tail, where H1 appended row 31 — expect exactly one conflict hunk there; keep both sides in the order 31, 32, 33. Every citation H2 adds (`cookiesLoadedFields`, `CookieJar.HorizonLogFields`, `TwitchLoginExpiry`, `regressedAfterWrite`, `platformsToRestoreAfterBrowserRefresh`, `AutoCookieService.LogProfileDirVerdict`, `cookieRefreshMechanismLabel`, the back-off trio and constants, the `§Cookie Jar` / `§Auto-Cookie Service` / `§ Refresh Service` references) resolved under the checker at the trial merge.

**Arc-close mutation summary:** 20 run / 18 caught / 2 survived — residual 2 (a real gap, fix dispatched) and residual 12 (pre-existing, documented). The seven task reviews ran 8+5, 4+3, 8+3, 7+2, 8+3, 7, 16+4.
