...
**Shipped, by commit** (branch `cookie-housekeeping-h1`, cut from `main` @ `383ed7d`; nine commits `dba7d20..49df05e`, then the arc-close commit):

| Task | Commit(s) | What landed |
|---|---|---|
| — | `dba7d20` | This plan and its spec, with the Fable plan review applied in place. |
| 1 (R1) | `ac1bf43` + fix `fddcf61` | `MarkStreamEnded` assigns `cd.liveContinuationOpen = false` under the lock it already holds (`internal/chat/downloader.go`), textually before `cancelCtx()` to match `Stop()`; Step 3a (RULED (a)) — `noteLivePollResult` takes `cd.mu` and opens the signal only while `cd.running && !cd.cancelFlag && !cd.streamEnded`. Tests: `TestMarkStreamEndedClosesLiveContinuationOpen`, `TestLatePollResultDoesNotReopenAfterMarkStreamEnded`; the fix round added `TestLatePollResultDoesNotReopenAfterStop` (isolates `!cd.cancelFlag` by restoring `running` after the real `Stop()`) and reworded the `MarkStreamEnded` comment to the true invariant (before `Unlock()`, not before `cancelCtx()`). |
| 2 (R2) | `e8cbc98` | `noteSegmentProgress` (`internal/worker/interruption.go`) extracted whole from the `onSegmentProgress` closure (`orchestrator_youtube.go`), byte-identical: same predicate, same two stores, same order; the `bool` return is discarded in production. `TestNoteSegmentProgress` brackets the stamp with `time.Now()` on both sides. |
| 3 (R3) | `c0712cc` + fix `5b7eb60` | `gqlBodySize` renders `"<n>-byte body"` at all five `gqlRequest` failure arms (`internal/twitch/api.go`); the retry Debug line logs `prev_status` (0 for a transport error), never `prev_err`; `gqlBaseRetryDelay` is a package `var` (a test seam, same value). The fix round replaced the whole-marker checks with `assertNoBodyFragment` (every 8-byte window of the stub body, at all four leak checks), scoped the `security.md` lede to Twitch GQL with the `FetchHLSMasterPlaylist` (`hls.go:310`) exception named, and corrected two `internal/worker` comments that still said the 401 routing was "a heuristic over a response body". |
| 4 (R4+R5) | `7008151` | The `Platforms` comment in `internal/config/types.go` states `detectCookiePlatforms`'s real rule (sidecar first, LOOSE `HasAny*` second, seeded only when both lists are empty, nothing automatic prunes); `*.css` / `*.svg` pinned LF (renormalise was a no-op — both blobs were LF). |
| 5 (R6) | `a6214c2` + fix `49df05e` | `internal/docs` citation checker: 317 file citations, 59 directories, 160 symbol/path pairs, 18 `§` references, 5 absence/state claims (keyed by sentence substring); five first-run rots fixed with the truth cited (the deleted `internal/errors` package ×3, `internal/goja/js/dom-real.js`, `validateChallengeOrigin` → `canonicalizeChallenge` incl. the orphaned `watch_page.go` stanza, and where `OnCancelAutoCookie` is declared / supplied / bound, in both `operations.md` and `user-interfaces.md`). The fix round added vacuity floors 250/40/120 counted at recognition, the five-entry `buildArtifacts` exemption (the exact contents of `internal/bgutils/embed/.gitignore`, scoped by path), rejection of reasonless allowlist entries by line number, and `TestCommentOnlySymbolDoesNotResolve`. |
| close | (this commit) | This section; `.gitattributes` gains the path-specific `internal/docs/citation_allowlist.txt text eol=lf`; `security.md`'s GQL bullet gains the `parseGQLBody` half-clause (true at `49df05e`: `internal/twitch/api.go` `parseGQLBody` renders only the decoded `errors[0].message`); field-test Part 5 row 28 (the arc's one user-visible change, R3's log shape). |

**Rulings recorded:** (a) Step 3a executed. The 8-byte fragment floor is exact by measurement (longest common substring between the stub body and any string the tests scan is 3 bytes; sub-8 windows are ordinary English) — do not slide it smaller. The `!cd.cancelFlag` conjunct is kept as defensive (not independently reachable: `Stop()` clears `running` in the same critical section) and pinned as written. The optional `len(citationPrefixes) == 7` / `len(specDocs) == 6` pins are NOT required (floors judged correctly calibrated). Five `buildArtifacts` entries ruled better than two.

**What actually changed in behaviour (arc-close trace):**

- **R1 is a contract fix, not an operator-visible timing change.** The field now closes at `orchestrator.go:522` (`chatDl.MarkStreamEnded()`) instead of at the chat loop's next wake. But the only production reader of `LiveContinuationOpen()` is `buildMayResume`'s closure (`internal/worker/interruption.go`, the `chatDl.LiveContinuationOpen()` read), consulted by the engine from inside `runLiveStreamDownload` (`orchestrator.go:449`) — which has already returned, been finalised and had its DB counts synced by the time `:522` runs. No production reader observes the earlier close; the spec's "the joint-idle gate then sees the signal closed the moment the orchestrator marks the stream ended" is true of the field and moot for the gate. What the arc bought is the signal's stated contract (a retired downloader carries no information; a late poll cannot re-latch it), held by three tests rather than by the loop's timing. No field-test row: nothing is observable.
- **R2 is byte-identical** (diffed at arc close: same predicate call, same two stores, same order, no added `time.Now()`).
- **R3 changes error STRINGS only**: all nine `gql …` format strings are unchanged; only the five `%s` arguments are (`string(respData)` → `gqlBodySize(respData)`). `classifyProbeErr` (`internal/worker/probe_classify.go`) still reads `http 5` / `http 4` out of the unchanged prefixes, and the auth prefix still contains neither (asymmetric default → keep waiting). All six `gqlRequest` callers do `if err != nil { return …, err }`; no `strings.Contains` on a GQL error exists outside `classifyProbeErr`. The one operator-visible change is the log/error shape — field-test Part 5 row 28.

**Residuals, each homed here unless stated:**

1. `noteLivePollResult`'s `cd.running` conjunct has no isolating test — dropping it survives the whole `internal/chat` package (arc-close mutant). Same class as the `!cd.cancelFlag` gap that got fix round 1; the loop itself clears `running` (`downloader.go`, `Start()`'s two exit paths), so a test sets `running = false` directly (no `Stop()`, no `MarkStreamEnded()`), polls, and asserts closed. **Open at arc close — closed by the fix dispatch that follows this commit (hash in the ledger).**
2. The `!cd.cancelFlag` conjunct: defensive, kept by ruling, pinned by `TestLatePollResultDoesNotReopenAfterStop`.
3. R2's production call site — the one-line `noteSegmentProgress(...)` call in `onSegmentProgress` (`orchestrator_youtube.go`) — is pinned by reading, not by a test: deleting it survives the whole `internal/worker` package. Inherent to the spec's premise (nothing in the tree calls `runLiveStreamDownload`); the extraction moved every testable line out of the closure, and that one line is all that remains.
4. R3's transport-error arm (`api.go`, `lastStatus = 0` beside the `gql request (%s): %w` error) is untested (F5, ruled observation): a mutant setting `lastStatus = 599` there survives. Go's `Do` error carries no response body, so no leak; closable by a `RoundTripper` that returns an error.
5. R3's sixth log site — the `twitch gql 429 honoring Retry-After` Debug line (keys `op`/`wait`) — is never exercised by the hygiene tests (the stub sets no `Retry-After`): a mutant adding `"body", string(respData)` to it survives. Same class as 4; closable by one stub variant that sets `Retry-After: 1` under the millisecond seam.
6. Each `NewAPI` + first request lazily starts an unstoppable rate-limiter ticker goroutine (F6, pre-existing); the five hygiene subtests leave five behind.
7. `parseGQLBody`'s modelled `errors[0].message` — now stated in the `security.md` bullet (home: that sentence).
8. The five first-run rot fixes — rots 3, 4 and 5 re-verified at arc close by reading (`internal/goja/js/dom-real.js` exists; `validateChallengeOrigin` has zero hits and `func canonicalizeChallenge` is at `watch_page.go:847`; `OnCancelAutoCookie` is the field at `setup_wizard.go:262`, bound at `app.go:628`, its callback supplied as an anonymous func around `CancelSetup()` at `tui_wiring.go:528`).
9. Optional input pins `len(citationPrefixes) == 7` / `len(specDocs) == 6` — ruled not required; would close the deleted-smaller-prefix / deleted-doc residual the count floors cannot see.
10. `buildArtifacts` is a hand-copy of `internal/bgutils/embed/.gitignore`; drift fails loudly (a newly named artifact reports as missing), the safe direction.
11. The checker's path check is `os.Stat`, case-insensitive on Windows: a wrong-case citation (`internal/twitch/API.go`) passes here and would fail on Linux (arc-close mutant survives). Zero case mismatches today (Task 5 review's one-off `git ls-files` comparison). Optional refinement: compare the last segment against `os.ReadDir` names exactly. No CI runs `go test`, so the Linux run does not exist yet either.
12. `buildArtifacts` is trusted: a `.go` entry added to it would silently skip that file's symbol pairs without tripping the 120 floor (arc-close mutant survives; requires editing the test itself). Optional one-line guard.
13. Claim (c) "Nothing automatic ever prunes it" checks the SET of writer files, not what a writer does: a prune inside a known writer survives by design (arc-close mutant). The sentence's truth rests on the four sites read at arc close — `services.go` seed (only when both lists are empty) and `FinishSetup` union, `config.go` legacy copy (only when `[cookies]` has none), `config_routes.go` operator replace.
14. Short-symbol (8 of 160 pairs, all backed by real declarations today) and short-heading word-prefix blind spots — ruled acceptable.
15. Directory allowlist keys keep the trailing slash; `fileFacts.resolves` compiles a regexp per literals-fallback call (at most 160 per run) — observations.
16. The reworded `watch_page.go:824-830` stanza still runs two paragraphs together with no blank `//` — pre-existing shape.
17. The sentence-substring keys of the five absence claims (`citations_test.go`, `absenceClaim.key` + `strings.Contains`) are what make Step 11's line drift harmless; a key 12c reworded fails loudly.

**Step 11 (the controller's, at the merge gate).** `main` is at `6d40b30` (Arc 12c). A `git merge-tree --write-tree 6d40b30 49df05e` dry run at arc close produced a clean tree with no conflict entries: H1 touches none of `docs/spec/data-and-storage.md` or `cmd/moombox/services.go`, and its only shared file, `internal/config/types.go`, is edited at `:209-221` (the `Platforms` comment) while 12c appends `Acquisition` after `DpapiFallback` at `:230-259`. What the checker re-reads after the merge: 12c's `data-and-storage.md` hunks at `:533` (+1 row, `Acquisition`, fresh citations), `:873`, `:887-888`, `:897-898`, `:901`, `:903`, `:953` (Timing / Auto-Cookie Service, `IsNoBrowserProfile`, detection) — new symbol/path pairs may fail and are fixed on the branch with the truth cited. The five claim keys are substrings, so the recorded drift (`:865`/`:1156` → `:864`/`:1151`) cannot break them. The H2 branch (`cookie-housekeeping-h2`, the `internal/cookies/**` items) is separate and merges later.

**Arc-close mutation summary:** 10 run / 3 caught / 7 survived — 1 real gap (residual 1) and 6 documented boundaries (residuals 3, 4, 5, 11, 12, 13). The five task reviews ran 43 (4+1, 4, 7+5, 0, 13+9).
