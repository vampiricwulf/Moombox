...
One rule keeps the ordering from silently costing a capability: **VISIONOS serves live streams over HLS only and never returns a `dashManifestUrl`** (verified 2026-08-24), while ANDROID_VR does. Returning the first adequate result therefore handed back an HLS-only live stream and skipped the callers' DASH-enrichment step, costing `--live-from-start` segment addressability on every anonymous live capture. So when an adequate result is live or upcoming and lacks a DASH manifest, the chain continues and **adopts the next client's manifest into the already-chosen result** rather than stopping early. A VOD, or a live result that already carries DASH, stops at the first client — no extra round trip.

This regression shipped and was caught the same day by `TestLivePublicExtraction`; `TestCookielessFallbackKeepsDash` is the offline gate.
