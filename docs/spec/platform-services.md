...
`RefreshService.checkTwitchAuth` (`internal/cookies/refresh.go`) is the same endpoint from the cookie side, and is likewise a check with no rotation. An empty `auth-token` returns "not authenticated" WITHOUT making a request: with no credential, nothing the endpoint replied could be a verdict about this install. There is no Twitch counterpart to the YouTube `Set-Cookie` capture — `processYouTubeSetCookies` is YouTube-only and `trackedCookieName` refuses any origin off the Google platform.

**There is no in-process Twitch keepalive, and that is a fact about the platform rather than a gap in this code.** yt-dlp never writes `auth-token` back; `references/chatterino7` refreshes Kick tokens and only DETECTS expiry for Twitch; the only issuer of a fresh `auth-token` is `passport.twitch.tv/login`, a browser sign-in. The headless refresh does navigate to `https://www.twitch.tv` on every periodic pass (`platformRefreshURLs`, `internal/cookies/autocookies.go`), and **whether that navigation renews the token is UNMEASURED**. The settling observation costs one run and has no surface yet: read the horizon through a test, or through a one-off call to `AuthCookieHorizonFor(PlatformTwitch)`, before and after a browser refresh, and compare. A timestamp, never a value. `AuthCookieHorizonFor` has test callers only — no UI and no log carries a horizon today — so this is a measurement to make, not a diagnostic to read.
...
**One notification per job.** `reportAuthDowngrade` latches across every trigger site (a THIRD flag, not a reuse of the per-site `warnedNoLogin` or the behaviour-changing `authRefused`), and the worker turns it into "Twitch chat is anonymous for {channel}" — `twitchChatDowngradeNotice` (`internal/worker/stream_processor_twitch.go`), `TypeWarning`, event `auth`, fields Channel / Job / Reason. The notice names the VIDEO consequence as well as the chat one, and that sentence is the part an operator acts on: THIS download is unaffected, because `Service.GetHLSMasterPlaylist` fetched its playback access token once at the start of the capture and the signed playlist is then polled without re-checking it. The NEXT capture starts anonymous — an anonymous token is served stitched ads, which the downloader correctly skips (the real feed is not served during the break), leaving a timestamp jump in the archive, and is refused outright on subscriber-only content. Dedup ACROSS jobs is deliberately absent: a second job an hour later with the same dead cookies must notify again, because by then the operator may believe they fixed it.
