...
| SegmentWorkers | int | 12 | `segment_workers` | Min: 1, **no max**. Concurrent segment fetches within a single download (catch-up on a live DASH stream, parallel VOD HLS). Values above `config.SegmentWorkersWarnThreshold` (16) log a startup warning and are flagged in both UIs' help text: a wide simultaneous fan-out to YouTube is a traffic shape that attracts bot detection. Measured 2026-08-15 on a live stream mid-archive: Moombox sustained 5.96 MB/s at the default 12 workers, against 2.86 MB/s for one `curl` connection and 11.28 MB/s for six parallel `curl` connections — the headroom the rolling-window catch-up rewrite exists to capture (a harness test of the same rewrite dropped 4.888s of batched catch-up to 0.84s). Not restart-required. |
