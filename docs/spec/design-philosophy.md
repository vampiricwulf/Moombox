...
- The **WebSocket broadcast system** uses a similar signal-driven approach with leading/trailing edge throttling. The first update in a quiet period sends immediately. Subsequent updates within 100ms are batched. During idle periods, the system does nothing.
