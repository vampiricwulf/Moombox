...
| Cookie Auto-Refresh Failed | Error | `runCookieRecovery`, the pass returned an error | The automatic refresh ran and failed |
...
| Parked Jobs Re-evaluated | Info | `OnCredentialsChanged` | N parked jobs were resumed after the signed-in account was re-observed. States no cause on purpose: this fires on the first authenticated observation of EVERY process, not only on a real account change, so "a different account was supplied" would often be false |
