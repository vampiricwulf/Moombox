...
**Status**: design approved; Phase 1 shipped; Phases 2–6 pending.
**Owner decision date**: 2026-04-26.

**Revision history:**

- 2026-04-26 — initial doc.
- 2026-04-26 — Phase 1 amendments (during implementation):
  - **Dependency**: switched from `bgutil-ytdlp-pot-provider` (GPL-3.0-only)
    to `bgutils-js` + `jsdom` direct (both MIT). Re-implemented the ~100-line
    `SessionManager.generateTokenMinter` flow inline in `src/server.js`.
    Reason: GPL-3.0 would force Moombox itself to GPL-3.0; bgutils-js by
    LuanRT is the underlying BotGuard library and is MIT-clean.
  - **Bundling**: dropped the esbuild-single-file approach. JSDOM reads
    static assets via `fs.readFileSync(__dirname + "../browser/...")` at
    runtime, which a bundled `dist/server.cjs` can't satisfy without
    non-trivial esbuild plugin work. Replaced with a tarball of
    `package.json + src/ + node_modules/` (production-only). The Go side
    extracts the tarball at first launch and runs `node src/server.js`.
    Tarball is 3.53 MB gzipped, well under the design-doc estimate.
  - **Embed file**: `internal/bgutils/embed/server.js.gz` →
    `internal/bgutils/embed/sidecar.tar.gz`. `node.exe.gz` (Phase 2)
    unchanged.
...
```
bgutil-sidecar/         (gitignored except for the build inputs)
...
├── sidecar.tar.gz                  # ~3.5 MB; gitignored, generated by bgutil-sidecar/build.mjs
...
`tools/fetch-node` and `bgutil-sidecar/build.mjs` before
...
2. Read `version.txt` from embed; compare to existing
   ...
   missing), extract `node.exe.gz` (gunzip-only) and `sidecar.tar.gz`
   (gunzip + tar) to cacheDir, then write the new `version.txt`.
   **Extraction uses Go's stdlib `archive/tar` + `compress/gzip` only**
   — the end user does NOT need a system `tar` binary. (`tar` is required
   at build time only, by `bgutil-sidecar/build.mjs`, where Windows 10+,
   Linux, and macOS all ship one.)
...
### 4.2 build pipeline (bgutil-sidecar/build.mjs)

> Original design used esbuild to produce a single bundled `server.js.gz`.
> Phase 1 implementation pivoted to a tarball of the production
> `node_modules/` after JSDOM's `__dirname`-relative asset reads broke the
> bundle. See the revision history at the top of this doc for the why.
> Current implementation lives in `bgutil-sidecar/build.mjs`; the gist:

```js
// 1. Sanity-check that production node_modules/ exists.
// 2. Spawn system tar:
//      tar --force-local -czf dist/sidecar.tar.gz \
//          package.json package-lock.json src node_modules
//    (cwd=bgutil-sidecar so paths inside the archive are relative to
//     the sidecar root)
// 3. Copy dist/sidecar.tar.gz to ../internal/bgutils/embed/sidecar.tar.gz
//    so go:embed picks it up at build time.
// 4. Print sizes.
```

Production install is ~17 MB unpacked → ~3.5 MB gzipped. esbuild is
deliberately NOT a dependency of this pipeline — runtime needs only Node
+ system tar (Windows 10+ ships one in `System32`).
...
### Phase 1 — Build inputs (1 day) — **COMPLETE (commit 3f5dd9d)**

Delivered:
- `bgutil-sidecar/package.json` + `package-lock.json` with deps
  `bgutils-js@^3.2.0` (MIT) and `jsdom@^27.4.0` (MIT). Note: did NOT use
  `bgutil-ytdlp-pot-provider` — that package is GPL-3.0-only and would
  contaminate Moombox's MIT license. The ~100-line `SessionManager.generateTokenMinter`
  flow is re-implemented inline in our `server.js`.
- `bgutil-sidecar/src/server.js` — ~250-line stdin/stdout JSON-RPC server
  (JSDOM bootstrap + dispatch loop with inflight tracking for graceful
  drain on stdin close).
- `bgutil-sidecar/build.mjs` — tarball pipeline (§4.2), no esbuild.

Verified end-to-end: `echo '{"id":1,"method":"generatePoToken",...}' | node src/server.js`
returns a real 144-char PO token from Google's WAA endpoint, not just the
websafe fallback. This is the milestone Option 2 (test.50–test.55) couldn't reach.
