...
func TestGenerateColdStartToken(t *testing.T) {
	token, err := GenerateColdStartToken("test-visitor-data", 1)
	if err != nil {
		t.Fatalf("GenerateColdStartToken: %v", err)
	}
	...
	// Verify it's valid base64url (with padding)
	data, err := base64.URLEncoding.DecodeString(token)
	if err != nil {
		t.Fatalf("decode token: %v", err)
	}

	// Verify structure: first byte should be 34
	if data[0] != 34 {
		t.Errorf("expected first byte 34, got %d", data[0])
	}

	// Second byte should be the payload length
	expectedPayloadLen := 8 + len("test-visitor-data") // header + identifier
	if int(data[1]) != expectedPayloadLen {
		t.Errorf("expected payload length %d, got %d", expectedPayloadLen, data[1])
	}
}
...
// ---------------------------------------------------------------------------
// GenerateColdStartToken — additional edge cases
// ---------------------------------------------------------------------------

func TestGenerateColdStartToken_ZeroClientState(t *testing.T) {
	token, err := GenerateColdStartToken("visitor", 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	// clientState defaults to 1 when 0 is passed; it's at payload byte index 3 (XOR'd)
	// We can verify the structure is valid (packet header + payload)
	if data[0] != 34 {
		t.Errorf("expected packet header 34, got %d", data[0])
	}
}

func TestGenerateColdStartToken_EmptyIdentifier(t *testing.T) {
	token, err := GenerateColdStartToken("", 1)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	// Payload should be just the 8-byte header (no identifier bytes)
	expectedPayloadLen := 8
	if int(data[1]) != expectedPayloadLen {
		t.Errorf("expected payload length %d, got %d", expectedPayloadLen, data[1])
	}
}

func TestGenerateColdStartToken_MaxLength(t *testing.T) {
	// 118 bytes should work
	id := strings.Repeat("x", 118)
	token, err := GenerateColdStartToken(id, 1)
	if err != nil {
		t.Fatalf("unexpected error for 118-byte identifier: %v", err)
	}
	if token == "" {
		t.Fatal("expected non-empty token")
	}

	// 119 bytes should fail
	id = strings.Repeat("x", 119)
	_, err = GenerateColdStartToken(id, 1)
	if err == nil {
		t.Fatal("expected error for 119-byte identifier")
	}
}

func TestGenerateColdStartToken_Deterministic_Structure(t *testing.T) {
	// Two calls with same identifier produce different tokens (random keys)
	t1, _ := GenerateColdStartToken("same", 1)
	t2, _ := GenerateColdStartToken("same", 1)
	if t1 == t2 {
		t.Error("expected different tokens due to random XOR keys")
	}
}
