...
func (m *WebPoMinter) MintAsWebsafeString(contentBinding string) (string, error) {
	...
	// Call mintCallback(encodedIdentifier) -> Uint8Array, timeboxed via the
	// shared interrupt helper. The mint executes server-supplied BotGuard
	// JS; a looping callback (anti-bot rotation) would otherwise block this
	// goroutine forever while pinning m.mu — wedging every later mint AND
	// Shutdown for the process lifetime.
	result, err := gojahelpers.CallWithTimeout(m.vm, DefaultMintTimeout, "mint", func() (goja.Value, error) {
		return m.mintCallback(goja.Undefined(), m.vm.ToValue(encoded))
	})
	if err != nil {
