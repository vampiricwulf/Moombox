...
- **PO tokens are minted the way yt-dlp mints them.** GVS tokens now come from the watch page's own BotGuard attestation challenge, bound by yt-dlp's content-binding rule (video ID for public playback, account/visitor identity otherwise), with mint provenance and job IDs in the logs so a 403 can be traced to its credential source. This is the prime suspect fix for premiere archives that produced empty streams; the next premiere is the field trial.
